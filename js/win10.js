...
window.Win10 = {
    ...
    _hideShortcut:function () {
        ...
        $('#win10 #win10-shortcuts').css('background','none');
        //console.log('oh,隐藏');
    },
    _showShortcut:function () {
        ...
        $('#win10 #win10-shortcuts').css('background','rgba(0,0,0,0.2)');
        //console.log('oh.显示了');
    },
    ...
    _init:function () {
        ...
        $("#win10 .desktop").click(function () {
            if($("#win10_toggle_left_nav span").hasClass('fa-angle-left')||$("#win10-menu").hasClass('opened')||!$("#win10_command_center").hasClass('hidden_right')){
                Win10.menuClose();
                Win10.commandCenterClose();
                //这里默认执行了2次显示桌面图标,是点击桌面区域必定触发的事件,主要是用在底部的2个按钮点击出的菜单后,当点击菜单区域外自动实现显示桌面图标的目标.
            }else{

            }

        });
        $('#win10_toggle_left_nav').click(function (){
            Win10.commandLeftToggle();
        });
        ...
        Win10.controlDSJwidth();//修改大数据的宽度和高度
        ...
        $(window).resize(function() {
            ...
            var uuu=$('#win10-shortcuts').css("visibility");
            switch(uuu){
                case 'visible':
                    Win10.controlDSJwidth();
                    break;
                case 'hidden':
                    Win10.clearDSJwidth();
                    break;
    ...
    menuClose: function () {
        ...
        $("#win10_toggle_left_nav").css('display','block');
        $(".win10-open-iframe").removeClass('hide');
        if($("#win10_toggle_left_nav span").hasClass('fa-angle-left')){
            this._showShortcut();
        }else{
    ...
    menuOpen: function () {
        ...
        $("#win10_toggle_left_nav").css('display','none');
    ...
    commandLeftOpen: function () {//桌面图标隐藏
        $("#win10_toggle_left_nav span").removeClass('fa-angle-left');
        $("#win10_toggle_left_nav span").addClass('fa-angle-right');
        $('#win10 #win10-shortcuts').css('visibility','hidden');
        this._hideShortcut();
        Win10.clearDSJwidth();
    },
    commandLeftClose: function () {//桌面图标显示
        $("#win10_toggle_left_nav span").removeClass('fa-angle-right');
        $("#win10_toggle_left_nav span").addClass('fa-angle-left');
        $('#win10 #win10-shortcuts').css('visibility','visible');
        this._showShortcut();
        Win10.controlDSJwidth();
    },
    renderShortcuts:function () {
        ...
        $("#win10 #win10-shortcuts .shortcut").each(function () {
            ...
            var bgwidth1=((x+1)*82+20);
            $('#win10 #win10-shortcuts').css('width',bgwidth1);
            y++;
            if(y>=h){
                y=0;
                x++;
    ...
    controlDSJwidth:function () {
        var leftDestopIconWidth=$('#win10-shortcuts').outerWidth();
        var mainBodyWidth=document.body.clientWidth;
        var left=mainBodyWidth-leftDestopIconWidth;
        $("#win10-desktop-scene").width(left);
        $("#win10-desktop-scene").css('left',leftDestopIconWidth);
        $(".layui-container").width(left-80);
    },
    clearDSJwidth:function () {//隐藏桌面图标去除掉定位和宽度限制.
         //var leftDestopIconWidth=$('#win10-shortcuts').outerWidth();
          var mainBodyWidth=document.body.clientWidth;
          var left=mainBodyWidth-140;
        $("#win10-desktop-scene").width('100%');
        $("#win10-desktop-scene").css('left','0');
        $(".layui-container").width(left);
    },
    ...
    commandLeftToggle: function () {
        if($("#win10_toggle_left_nav span").hasClass('fa-angle-left')){
            this.commandLeftOpen();//隐藏桌面图标
        }else{
            this.commandLeftClose();//显示桌面图标
        }
    },
    ...
    aboutUs: function() {
        ...
        layer.open({
            ...
            '<p>支持组件:<br>layer、jquery、animated.css、font-awesome</p>' +
                '<p>填写相关的补充信息</p>'+
            '</div>'
        });
    },
